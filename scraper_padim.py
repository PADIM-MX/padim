...
def detectar_moneda(prop: dict) -> tuple:
    ...
    if re.search(r'us\s*\$|usd|dls|d[oó]lar(?:es)?\b', texto, re.I):
...
def main():
    ...
    # Cargar fingerprints existentes para evitar duplicados entre ejecuciones
            ...
            for line in f:
                ...
                if line:
                    try:
                        ...
                        pass
        log.info(f"Fingerprints cargados: {len(seen)}")

    # Ejecutar portales
    for name, cfg in config["portals"].items():
        if cfg.get("enabled", True):
            try:
                run_portal(name, cfg, settings, seen)
            except Exception as e:
    ...
    log.info(f"\n═══ COMPLETADO ═══")
    log.info(f"Propiedades totales en DB: {total}")
